...
def convert(client, attack_type, weights, rng=np.random.default_rng()):
    """
...
def step(self, weights, return_weights=False):
    ...
    if self.attack_type == "random":
        grad = ymir.path.tree.uniform(weights, low=-10e-3, high=10e-3, rng=self.rng)
    else:
        grad = ymir.path.tree.sub(weights, self.prev_weights)
        if "advanced" in self.attack_type:
            grad = ymir.path.tree.add_normal(grad, loc=0.0, scale=10e-4, rng=self.rng)
    self.prev_weights = weights
    return 0.1, ymir.path.tree.add(weights, grad) if return_weights else grad, self.batch_size
