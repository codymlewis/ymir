    ...
    def __call__(self, params, rng=np.random.default_rng()):
        ...
        for i in idx:
            ...
            for _ in range(self.clients[i].epochs):
                grads, self.clients[i].opt_state, updates = self.update(p, self.clients[i].opt_state, *next(self.clients[i].data))
...
class Network:
    ...
    def __init__(self, opt, loss, C=1.0):
        self.clients = []
        self.controllers = {}
        self.server_name = ""
        self.opt = opt
    ...
    def add_controller(self, name, con_class=Controller, is_server=False):
        ...
        self.controllers[name] = con_class(self.opt, self.loss, self.C)
        if is_server:
