...
class TestTreeFunctions(parameterized.TestCase):
    def setUp(self):
        self.length = 50
    ...
    def test_tree_uniform(self, low, high):
        uniform_params = ymirlib.tree_uniform(self.params, low=low, high=high)
    ...
    @parameterized.named_parameters(
        [
            {"testcase_name": f"_{loc=}_{scale=}", "loc": loc, "scale": scale}
            for loc, scale in [(0.0, 1.0), (0.5, 0.7), (-0.5, 0.5), (2, 10), (-1000, 10)]
        ]
    )
    def test_tree_add_normal(self, loc, scale):
        normal_params = ymirlib.tree_add_normal(self.params, loc=loc, scale=scale)
    ...
    @parameterized.named_parameters(
        [
            {"testcase_name": f"_{mul=}", "mul": mul}
            for mul in [2, -1, 0, 1, 3, 5]
        ]
    )
    def test_tree_mul(self, mul):
        mul_params = ymirlib.tree_mul(self.params, mul)
    ...
    @parameterized.named_parameters(
        [
            {"testcase_name": f"_{a=}_{b=}", "a": a, "b": b}
            for a, b in itertools.product([2, -1, 0, 1, 3, 5], repeat=2)
        ]
    )
    def test_tree_add(self, a, b):
        add_params = ymirlib.tree_add(
        ...
        chex.assert_tree_all_close(add_params, Params(w=jnp.full(self.length, a + b), b=jnp.full(self.length, a + b)))
